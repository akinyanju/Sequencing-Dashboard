...
#------------------------------------------------------------------
# Function: find_illumina_nonarchive_qc_directories
# Purpose : Find Illumina project directories in non-archival
#           directories using .settings.json. Logs structured messages
#           and handles missing files gracefully. Some alias are similar among applications
#           therefore, a physical application folder must be present. 
#------------------------------------------------------------------
find_illumina_nonarchive_qc_directories() {
  local result_dirs=()
  local app_lower
  app_lower=$(echo "$Application" | tr '[:upper:]' '[:lower:]')
  local search_dir="$QCdir_illumina_nonarchive"
  ...
  declare -A app_aliases=(
    ...
    [hic]=",hic,"
    [ctp]=",pdx-wgs,ctp,"
    [pdxrnaseqr2]=",cellranger,pdxrnaseqr2,"
    [rnaseq]=",mrna,total-rna,rnaseq,"
    [rrbs]=",rrbs,"
    [wgbs]=",wgbs,"
    [chipseq]=",chip,chipseq,"
    [pdxrnaseq]=",pdx-mrna,pdxrnaseq,"
    [pdxwgs]=",pdx-wgs,pdxwgs,"
    [rnaseqr2]=",cellranger,rnaseqr2,"
    [wes]=",wes,"
    [wgs]=",wgs,chia-pet,"
    [basic]=",amplicon,other,basic,"
  )

  local aliases="${app_aliases[$app_lower]}"

  if [[ -z "$aliases" ]]; then
    log_warn "[$Application] Not recognized for Illumina directory matching"
    return 1
  fi

  while IFS= read -r json_path; do
    local proj_dir app_value app_value_lc
    proj_dir=$(dirname "$json_path")
    app_value=$(grep -oP '"application"\s*:\s*"\K[^"]+' "$json_path" | head -n 1)
    app_value_lc=$(echo "$app_value" | tr '[:upper:]' '[:lower:]')

    if [[ -n "$app_value_lc" ]]; then
      case "$app_lower" in
        chic)
          if [[ "$app_value_lc" == "other" && -d "$proj_dir/chic" ]]; then
            result_dirs+=("$proj_dir/chic")
          fi
          ;;
        pdxrnaseqr2)
          if [[ "$app_value_lc" == "cellranger" && -d "$proj_dir/pdxrnaseqr2" ]]; then
            result_dirs+=("$proj_dir/pdxrnaseqr2")
          fi
          ;;
        ctp)
          if [[ ",$aliases" == *",$app_value_lc,"* && -d "$proj_dir/ctp" ]]; then
            result_dirs+=("$proj_dir/ctp")
          fi
          ;;
        pdxwgs)
          if [[ ",$aliases" == *",$app_value_lc,"* && -d "$proj_dir/pdxwgs" ]]; then
            result_dirs+=("$proj_dir/pdxwgs")
          fi
          ;;
        *)
          if [[ ",$aliases" == *",$app_value_lc,"* ]]; then
            result_dirs+=("$proj_dir/$app_lower")
          fi
          ;;
      esac
    fi
  ...
  log_info "[$Application] Found ${#result_dirs[@]} matching projects in $search_dir"
  printf "%s\n" "${result_dirs[@]}"
}
#------------------------------------------------------------------
# Function: find_illumina_archive_qc_directories
# Purpose : Find Illumina project directories in archival
#           directories using .settings.json. Logs structured messages
#           and handles missing files gracefully.
#------------------------------------------------------------------
find_illumina_archive_qc_directories() {
  local result_dirs=()
  local app_lower
  app_lower=$(echo "$Application" | tr '[:upper:]' '[:lower:]')
...
#------------------------------------------------------------------
# Function: find_longread_nonarchive_qc_directories
# Purpose : Find PacBio or ONT project directories in non-archival
#           directories using .settings.json. Logs structured messages
#           and handles missing files gracefully.
#------------------------------------------------------------------
find_longread_nonarchive_qc_directories() {
  local result_dirs=()
  local search_dir
...
#------------------------------------------------------------------
# Function: find_longread_archive_qc_directories
...
#------------------------------------------------------------------
find_longread_archive_qc_directories() {
  local result_dirs=()
  local search_dir="$QCdir_archive"
  local app_lower=$(echo "$Application" | tr '[:upper:]' '[:lower:]')
...
whitelist_QCdir() {
  ...
  if [[ "$day_5_countdown" -ge 5 ]]; then
    ...
    if [[ "$Application" == "PacBio" || "$Application" == "ONT" ]]; then
      find_longread_archive_qc_directories > "$qcdir_file_list"
    else
      find_illumina_archive_qc_directories > "$qcdir_file_list"
    fi        
  ...
  # Step 4: If database exists, look for newly added non-archived directories
    ...
    if app_match | grep -Fxq "$Application" && [[ -f "$qcdir_file_list" ]]; then
      if [[ -f "$pulling_undelivered_qc_flag" ]]; then
        rm "$pulling_undelivered_qc_flag"
      fi
      ...
      if [[ "$Application" == "PacBio" || "$Application" == "ONT" ]]; then
        find_longread_nonarchive_qc_directories > "$qcdir_file_list"
      else
        find_illumina_nonarchive_qc_directories > "$qcdir_file_list"
      fi
  ...
  for n in $(seq 1 "$ProjCount"); do
    ...
    if [[ -f "$ProjDir_list/$SETJSONFILE" ]]; then
      ...
      if [[ -n "$projectApplication" ]]; then
        ...
        if [[ "$Application" == "atacseq"     && ",ATACSeq,ATAC,atacseq,"        == *",$projectApplication,"* ]] ||
           [[ "$Application" == "chic"        && ",Other,chic,"                  == *",$projectApplication,"* ]] ||
           [[ "$Application" == "hic"         && ",HiC,hic,"                     == *",$projectApplication,"* ]] ||
           [[ "$Application" == "ctp"         && ",PDX-WGS,ctp,"                 == *",$projectApplication,"* ]] ||
           [[ "$Application" == "pdxrnaseqR2" && ",cellranger,pdxrnaseqR2,"      == *",$projectApplication,"* ]] ||
           [[ "$Application" == "rnaseq"      && ",mRNA,Total-RNA,rnaseq,"       == *",$projectApplication,"* ]] ||
           [[ "$Application" == "rrbs"        && ",RRBS,rrbs,"                   == *",$projectApplication,"* ]] ||
           [[ "$Application" == "wgbs"        && ",WGBS,wgbs,"                   == *",$projectApplication,"* ]] ||
           [[ "$Application" == "chipseq"     && ",ChiP,ChIPSeq,chipseq,"        == *",$projectApplication,"* ]] ||
           [[ "$Application" == "pdxrnaseq"   && ",PDX-mRNA,pdxrnaseq,"          == *",$projectApplication,"* ]] ||
           [[ "$Application" == "pdxwgs"      && ",PDX-WGS,pdxwgs,"              == *",$projectApplication,"* ]] ||
           [[ "$Application" == "rnaseqR2"    && ",cellranger,rnaseqR2,"         == *",$projectApplication,"* ]] ||
           [[ "$Application" == "wes"         && ",WES,wes,"                     == *",$projectApplication,"* ]] ||
           [[ "$Application" == "wgs"         && ",WGS,ChIA-PET,wgs,"            == *",$projectApplication,"* ]] ||
           [[ "$Application" == "basic"       && ",Amplicon,Other,basic,"        == *",$projectApplication,"* ]] ||
           [[ "$Application" == "ONT"         && ",ONT,ont,"                     == *",$projectApplication,"* ]] ||
           [[ "$Application" == "PacBio"      && ",PacBio,pacbio"                == *",$projectApplication,"* ]]; then
...
QCdir_updatelist() {
  ...
  if [[ "$skip" -eq 1 ]]; then
    #Also double check that the unprocessed QC directory is not yet mistakently whitelisted
    if grep -q "$ProjDir" "$qcdir_file_list" || grep -q "$ProjDir" "$qcdir_file_update_list"; then
      grep -v "$ProjDir" "$qcdir_file_update_list" >> "$nonarchive_tmp_file"
      mv "$nonarchive_tmp_file" "$qcdir_file_update_list"
...
Instrument_SN)
  ...
  cat "$metrics_csv_tmp" | \
    ...
    if pivot_long_longread > "$metrics_csv_tmp"; then
        mv "$metrics_csv_tmp" "$metrics_csv"
    else
        log_warn "[$Application] Pivoting failed for $metrics_csv"
        rm -f "$metrics_csv_tmp"
        return 1
    fi
}
  ...
  for n in $(seq 1 "$ProjTotal"); do
    ...
    if [[ -f "$pulling_undelivered_qc_flag" || "$day_5_countdown" -ge 5 ]]; then
      ...
      if ! QCdir_updatelist; then
        #this if block has been earlier handled. It is repeated here just to enhance firewall
        if grep -q "$ProjDir" "$qcdir_file_list" || grep -q "$ProjDir" "$qcdir_file_update_list"; then
          grep -v "$ProjDir" "$qcdir_file_update_list" >> "$nonarchive_tmp_file"
          mv "$nonarchive_tmp_file" "$qcdir_file_update_list"
          grep -v "$ProjDir" "$qcdir_file_list" > "$nonarchive_tmp_file"
...
duckDB_call_seq() {
  ...
  # Check row counts for early exit
  before_count=$(get_database_row_count_if_table_exists "$duckDB_PATH" "sequencing_metrics")
  [[ ! "$before_count" =~ ^[0-9]+$ ]] && before_count=0

  csv_count=$(awk 'END {print NR-1}' "$seq_metrics_file")
  [[ "$csv_count" -lt 0 ]] && csv_count=0

  if [[ "$before_count" -eq "$csv_count" && "$csv_count" -gt 0 ]]; then
    echo "[$(date)] Skipping import — CSV data rows match database ($csv_count rows)." >> "$duckDB_logfile"
    return 0
  fi
...
SELECT
  ...
  FullPath,
  Platform,
  CAST(Reads AS BIGINT) AS Reads,
  CAST(Bases AS BIGINT) AS Bases,
  CAST(Bytes AS BIGINT) AS Bytes,
  DeliveryDirectory,
  CAST(SampleSize AS INTEGER) AS SampleSize,
  TRY_CAST(PolymeraseReadLength_bp_Mean AS DOUBLE) AS PolymeraseReadLength_bp_Mean
FROM $seq_tmp_table_raw;
...
CREATE TABLE IF NOT EXISTS sequencing_metrics (
  ...
  UNIQUE (
    Month, Year, Project, Reads, Bases, Bytes
  )
);
...
EOF
      ...
      echo "    a. Total rows before import: $before_count" >> "$duckDB_logfile"
      echo "    b. Total rows after import: $after_count" >> "$duckDB_logfile"

      if [[ "$new_rows" -eq 0 ]]; then
        echo "    ⚠️  No new rows imported (all already existed)." >> "$duckDB_logfile"
      else
        echo "     Imported $new_rows new rows." >> "$duckDB_logfile"
      fi
...
SELECT 'qc_illumina_metrics' AS Source, Application, Investigator_Folder, COUNT(DISTINCT Sample_Name) AS Sample_Count
...
SELECT 'qc_pacbio_metrics' AS Source, Application, Investigator_Folder, COUNT(DISTINCT Sample_Name) AS Sample_Count
...
SELECT 'qc_ont_metrics' AS Source, Application, Investigator_Folder, COUNT(DISTINCT Sample_ID) AS Sample_Count
FROM qc_ont_metrics
WHERE Application IS NOT NULL AND Investigator_Folder IS NOT NULL
GROUP BY Application, Investigator_Folder;
EOF
...
# ─────────────────────────────────────────────────────────────
destination_server() {
  #── Declare all counters as integers ───────────────────────
  ...
  declare -i pb_rows_before=0 pb_rows_after=0 pb_delta_rows=0
  declare -i ont_rows_before=0 ont_rows_after=0 ont_delta_rows=0
  declare -i pb_samples_before=0 pb_samples_after=0 pb_delta_samples=0
  declare -i ont_samples_before=0 ont_samples_after=0 ont_delta_samples=0

  #── Bail if DB is missing ──────────────────────────────────
  if [[ ! -f "$duckDB_PATH" ]]; then
  ...
  pb_rows_after=$(run_count "$duckDB_PATH" "SELECT COUNT(*) FROM qc_pacbio_metrics;")
  pb_samples_after=$(run_count "$duckDB_PATH" "SELECT COUNT(DISTINCT Sample_Name) FROM qc_pacbio_metrics;")
  ont_rows_after=$(run_count "$duckDB_PATH" "SELECT COUNT(*) FROM qc_ont_metrics;")
  ont_samples_after=$(run_count "$duckDB_PATH" "SELECT COUNT(DISTINCT Sample_ID) FROM qc_ont_metrics;")

  #── 2) If snapshot exists, compute before + deltas ─────────
  if [[ -s "$duckDB_lastpush" ]]; then
    ...
    pb_rows_before=$(run_count "$duckDB_lastpush" "SELECT COUNT(*) FROM qc_pacbio_metrics;")
    pb_samples_before=$(run_count "$duckDB_lastpush" "SELECT COUNT(DISTINCT Sample_Name) FROM qc_pacbio_metrics;")
    ont_rows_before=$(run_count "$duckDB_lastpush" "SELECT COUNT(*) FROM qc_ont_metrics;")
    ont_samples_before=$(run_count "$duckDB_lastpush" "SELECT COUNT(DISTINCT Sample_ID) FROM qc_ont_metrics;")
    ...
    (( pb_delta_rows     = pb_rows_after     - pb_rows_before     )) || true
    (( pb_delta_samples  = pb_samples_after  - pb_samples_before  )) || true
    (( ont_delta_rows    = ont_rows_after    - ont_rows_before    )) || true
    (( ont_delta_samples = ont_samples_after - ont_samples_before )) || true
    ...
    pb_delta_rows=$pb_rows_after
    pb_delta_samples=$pb_samples_after
    ont_delta_rows=$ont_rows_after
    ont_delta_samples=$ont_samples_after

    email_header="Initial load completed at $(date)."
  fi

  #── 3) Skip if no new rows ─────────────────────────────────
  if [[ -s "$duckDB_lastpush" ]] &&
     (( qc_delta_rows == 0 && seq_delta_rows == 0 && pb_delta_rows == 0 && ont_delta_rows == 0 )); then
